# Hi, I'm Mic 👋


Senior IT Management & Service Desk Leader • USAF Veteran • Cyber Integrations & Information and Audio Video Systems


[![GitHub followers](https://img.shields.io/github/followers/MicahMicWilson?label=Follow&style=social)](https://github.com/MicahMicWilson)
[![Visits](https://komarev.com/ghpvc/?username=MicahMicWilson)](https://github.com/MicahMicWilson)


I build resilient IT service organizations and reliable media/broadcast systems. After 23 years in the U.S. Air Force leading cross‑functional teams, I specialize in ITSM, infrastructure modernization, and secure communications—aligning operations with ITIL 4, ISO, and NIST frameworks. I love shipping practical solutions, mentoring teams, and turning chaos into clarity.


- 🔭 **Now**: Transitioning to civilian IT leadership; exploring media systems engineering and platform operations.
- 🎮 **Fun**: Lifelong gamer (NBA 2K fan) who geeks out on automation and clean incident/problem flows.
- 🛰️ **Background**: Broadcast engineering, SCIF/ICD‑705 projects, VIIDS security systems, and enterprise service desks.
- 🌱 **Learning**: Modern DevOps tooling, observability, and cloud‑native service management.


## Core Skills
- **ITSM**: Incident, Request, Change, Problem • CSI • SLAs/XLAs • Knowledge Mgmt
- **Infra & Sec**: Networks, Windows/Linux, identity/access, SIEM • NIST/ISO controls
- **Media Systems**: ATSC-aligned workflows, signal chains, live production reliability
- **Leadership**: Mentorship, team building, vendor mgmt, stakeholder engagement


## Featured Projects
- **Ops Runbook Templates** — Practical checklists for incidents, changes, and on‑call.
- **Media Signal Monitor** — Proof‑of‑concept for monitoring/alerting in broadcast chains.
- **Service Desk Dashboards** — KPI/OKR starter pack for queue health and CSAT.


> Pin the repos above to your profile for quick access.


## Get in Touch
- 💼 LinkedIn: https://www.linkedin.com/in/micahmwilson
- ✉️ Email: mic_h_wilson@outlook.com


---


### Quick Start
```bash
# Clone one of my public projects
git clone https://github.com/MicahMicWilson/<project>
cd <project>
